...
function* login(action) {
  try {
    const credentials = yield call(rest.login, action.email, action.password);
...
function* password(action) {
  try {
    const refreshToken = localStorage.getItem("refreshToken");
    const credentials = yield call(rest.password, refreshToken, action.password);
    localStorage.setItem("refreshToken", credentials.refreshToken);
    connection.set(credentials.accessToken);
    history.goBack();
  } catch (err) {
...
function* register(action) {
  try {
    const credentials = yield call(rest.register, action.account);
    localStorage.setItem("refreshToken", credentials.refreshToken);
    connection.set(credentials.accessToken);
    yield put(actions.accountLoginUpdated(connection.account()));
    history.push("/");
...
function* forgot(action) {
  try {
    yield call(rest.forgot, action.email);
  } catch (err) {
    yield put(generateError(err));
  }
}
...
function* reset(action) {
  try {
    const credentials = yield call(rest.reset, action.token, action.password);
    localStorage.setItem("refreshToken", credentials.refreshToken);
    connection.set(credentials.accessToken);
    yield put(actions.accountLoginUpdated(connection.account()));
    history.push("/");
...
function* logout(action) {
  try {
    const refreshToken = localStorage.getItem("refreshToken");
    yield call(rest.logout, refreshToken);
  } catch (err) {
